...
class Notificacao extends PHPFrodo
{
    ...
    public function mercadopago(){
        ...
        if(isset($this->data[0]) && !empty($this->data[0])){
            ...
            // chama o helper que carrega o autoload do MP
            $this->helper('mp');
            $url = 'https://api.mercadopago.com/v1/payments/:' . $_REQUEST['data_id'] .'?access_token=' . $token;
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_POST, 1);
            ...
            if(isset($payment->status) && isset($payment->external_reference)){
                ...
                if ($status == 'rejected' || $status == 'cancelled' || $status == 'refunded') {
                    $status_n = 7;
                    $this->update('pedido')
                        ->set(array('pedido_pay_situacao', 'pedido_status'), array($status_n, $status_n))
                        ->where("pedido_id = $external_reference")
                        ->execute();
                    $this->notificarAdmin();
                    $this->notificarCliente();
