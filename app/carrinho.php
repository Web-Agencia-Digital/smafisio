...
class Carrinho extends PHPFrodo
{
    ...
    public function getCarrinho()
    {
        if (isset($_SESSION['cart']) && count($_SESSION['cart']) >= 1) {
            ...
            foreach ($this->cart as $k => $v) {
                ...
                if (isset($this->cart[$k]['atributos']) && !empty($this->cart[$k]['atributos'])) {
                    ...
                    if (isset($atrs[3]) && !empty($atrs[3])) {
                        ...
                        $att_now = $this->getAttr_item($att_id, $id);
                        $att_now_preco = $att_now[0];
                        $att_now_item_preco = $att_now[3];
                        $att_now_preco_desconto = $att_now[4];
                        $this->cart[$k]['item_estoque'] = intval($att_now[2]);
                        $this->cart[$k]['atributo_qtde'] = intval($att_now[2]);
                        $this->cart[$k]['item_valor_original'] = $this->cart[$k]['item_preco'] + $this->cart[$k]['item_desconto'];
                        if (intval($att_now_preco) > 0) {
                            $this->cart[$k]['item_valor_original'] = $att_now_preco;
                ...
                if ($this->cart[$k]['item_qtde'] > $this->cart[$k]['item_estoque']) {
                    $this->cart[$k]['item_qtde'] = $this->cart[$k]['item_estoque'];
                }
            ...
            $this->assign('showEmptyCart', 'hide');
            ...
            foreach ($this->cart as $k => $v) {
                ...
                if (isset($this->cart[$k]['atributos']) && !empty($this->cart[$k]['atributos'])) {
                    ...
                    if (isset($atrs[3]) && !empty($atrs[3])) {
                        $att_id = intval($atrs[3]);
                        $att_now = $this->getAttr_item($att_id, $id);
                        $att_now_preco = floatval($att_now[0]);
                        $att_now_item_preco = floatval($att_now[3]);
                        $att_now_preco_desconto = floatval($att_now[4]);
                        $this->cart[$k]['item_estoque'] = intval($att_now[2]);
                        $this->cart[$k]['atributo_qtde'] = intval($att_now[2]);
                        if (intval($att_now_preco) > 0) {
                            $this->cart[$k]['item_preco'] = ($att_now_preco + $att_now_item_preco) - $att_now_preco_desconto;
                    ...
                    if ($this->result()) {
                        $this->cart[$k]['item_preco'] = ($this->data[0]['item_preco']) - $this->data[0]['item_desconto'];
                        $this->cart[$k]['item_valor_original'] = floatval($this->data[0]['item_preco']);
                        $this->cart[$k]['item_estoque'] = intval($this->data[0]['item_  estoque']);
                    }
                }
                if ($this->cart[$k]['item_qtde'] > $this->cart[$k]['item_estoque']) {
                    $this->cart[$k]['item_qtde'] = $this->cart[$k]['item_estoque'];
    ...
    public function getAttr_item($iattr_id, $item)
    {
        $this->select('item_estoque,relatrr_qtde,item_preco,relatrr_preco,item_desconto')
            ->from('item')
            ->join('relatrr', 'relatrr_item = item_id', 'LEFT')
            ->where("relatrr_iattr = $iattr_id AND relatrr_item = $item")
            ->execute();
        if ($this->result()) {
            $estoque = intval($this->data[0]['item_estoque']);
            $att_estoque = intval($this->data[0]['relatrr_qtde']);
            $att_preco = $this->data[0]['relatrr_preco'];
            if (preg_match_all('/\./', $att_preco, $m)) {
                if (isset($m[0]) && count($m[0]) >= 2) {
                    $att_preco = floatval(preg_replace('/\./', '', $att_preco, 1));
                }
            }
            return array($att_preco, $estoque, $att_estoque, $this->data[0]['item_preco'], $this->data[0]['item_desconto']);
        }
    }
    ...
    function nCalculo()
    {
        if (isset($_SESSION['cart'])) {
            ...
            foreach ($this->data as $d) {
                $lid = $d['item_id'];
                $cubico += (floatval($d['item_altura']) * floatval($d['item_largura']) * floatval($d['item_comprimento'])) / 6000;
                $p = $p + (floatval($d['item_peso']) * floatval($d['item_qtde']));
                $a = $d['item_altura'];
                $l = $d['item_largura'];
    ...
    function adicionar()
    {
        if (isset($this->uri_segment[2])) {
            ...
            if (isset($_POST['attr']) && !empty($_POST['attr'])) {
                ...
                $strAttr_list = $atributos;
                $strAttr = implode("|", $strAttr);
            }
            $attr_preco_extra = 0;
            foreach ($strAttr_list as $extra) {
                $attr_preco_extra += $extra[4];
            }
            if (isset($_POST['id']) && !empty($_POST['id'])) {
                ...
                if ($this->checkItemCart($strAtrrPed) == false) {
                    ...
                    $this->getItem();
                    if ($_SESSION['cart'][$this->cart_id]['atributo_qtde'] < $this->estoque) {
                        $this->item['item_preco'] = ($this->item['item_preco'] + $attr_preco_extra);
    ...
    public
    function checkItemCart($strAtrrPed = "")
    {
        $incart = false;
        if (isset($_SESSION['cart']) && count($_SESSION['cart']) >= 1) {
                ...
                if (isset($_SESSION['cart'][$k]['item_id']) && $_SESSION['cart'][$k]['item_id'] == $this->item_id) {
                    if (isset($_SESSION['cart'][$k]['atributo_ped'])) {
                        if ($_SESSION['cart'][$k]['atributo_ped'] == $strAtrrPed) {
                            ...
                            $this->cart_item_qtde = $_SESSION['cart'][$this->cart_id]['item_qtde'];
                            $incart = true;
                        }
                    }
                }
    ...
    public
    function getAttr($iattr_id)
    {
        $this->select()
            ->from('atributo')
            ->join('iattr', 'iattr_atributo = atributo_id')
            ->join('relatrr', 'iattr_atributo = relatrr_iattr', 'LEFT')
            ->where("iattr_id = $iattr_id")
            ->execute();
        if ($this->result()) {
            return array($this->data[0]['atributo_nome'], $this->data[0]['iattr_nome'], $this->data[0]['atributo_id'], $this->data[0]['iattr_id'], $this->data[0]['relatrr_preco']);
        }
    }
    ...
    function incrementa()
    {
        ...
        if (isset($this->uri_segment[2])) {
            ...
            if (isset($_SESSION['cart'][$this->cart_id]['item_id'])) {
                ...
                $this->item_id = intval($_SESSION['cart'][$this->cart_id]['item_id']);
                $this->getItem();

                if (isset($_SESSION['cart'][$this->cart_id]['atributo_qtde'])) {
                    if ($_SESSION['cart'][$this->cart_id]['item_qtde'] < $this->estoque) {
                        $_SESSION['cart'][$this->cart_id]['item_qtde']++;
                    ...
                    if ($_SESSION['cart'][$this->cart_id]['item_qtde'] < $this->estoque) {
                        ...
                        $this->getTotal();
                        $this->estoque = $_SESSION['cart'][$this->cart_id]['item_estoque'];
                        echo "{\"aqui\":\"1\", \"total\":\"$total\",\"qtde\":\"$qtde\",\"attr_qtde\":\"null\",\"total_compra\":\"" . $this->_money($this->total_compra) . "\",\"estoque\":\"$this->estoque\"}";
                        $_SESSION['cart'][$this->cart_id]['item_estoque']--;
                    } else {
                        echo "{\"estoque\":\"0\"}";
                    }
    ...
    function getTotal()
    {
        if (isset($_SESSION['cart']) && count($_SESSION['cart']) >= 1) {
            ...
            $_SESSION['__TOTAL__COMPRA__'] = $this->total_sem_desconto;
            $r = new Route;
            if ($r->get() == "FINALIZAR") {
                $this->getTotalCupom();
            }
        ...
        if (isset($_SESSION['cupom']['desconto']) && $_SESSION['cupom']['desconto'] >= 1) {
            ...
            if ($this->cupom_tipo == 1) {
                ...
                if ($this->valor_frete >= 1) {
                    ...
                    $this->cupom_msg = "<p class='text-success'>Cupom Frete Grátis!</p>";
                } else {
