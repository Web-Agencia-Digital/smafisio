$(function () {
    ...
    $('.moneyUs').mask('000000000000000.00', {
        reverse: true,
        onChange: function (cep, event, currentField, options) {
            var eid = currentField[0].id;
            var id = $('#'+eid).data('id');
            var att = $('#'+eid).data('att');
            bindAddAttForce(id,att);
        }
    });
    ...
    var popcontent = '<p>Para uma melhor exibição, cadastre apenas o t?tulo do item. Exemplo:<br /> <b> Smartphone Motorola</b>';
    ...
    var popcontent3 = '<p>Esta opção determina se o Item aparecer? ou n?o no site.<p>';
    $('#item_show').popover({
        placement: 'right',
        title: 'Item Ativo',
        html: true,
    ...
    $('#btn-remove-cancel').live('click', function (e) {
        ...
        notify('<h1>Seleção cancelada</h1>');
    })
    //seleciona todas 
...
function bindAddAttForce(id,att) {
    var iattr_id = id;
    var atributo_id = att;
...
function bindAddAtt() {
    $('.addAttr input').live('change', function () {
        var iattr_id = $(this).data('id');
        var atributo_id = $(this).data('att');
        var iattr_qtde = $.trim($('#qtde-' + iattr_id).val());
        var iattr_preco = $.trim($('#preco-' + iattr_id).val());
        var item_id = $.trim($('#item_id').val());
        var url = baseUri + '/admin/item/addAttr/';
